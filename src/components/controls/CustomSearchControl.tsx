...
const CustomSearchControl: React.FC<CustomSearchControlProps> = ({
  ...
  const MarkerWithValue: React.FC<{ device: MeasurementDevice }> = ({
    ...
    // Style pour le contour bleu des données corrigées (uniquement si le feature flag est false)
    const correctionBorderStyle =
      device.source === "atmoMicro" && device.has_correction && !featureFlags.correctedCheck
        ? {
            boxShadow: `
    ...
    return (
      <div
        ...
        style={{ width: "32px", height: "32px", ...correctionBorderStyle }}
      >
        {/* Image du marqueur */}
        <img
          src={markerPath}
        ...
        {/* Indicateur de valeur corrigée pour AtmoMicro (uniquement si le feature flag est true) */}
        {device.source === "atmoMicro" && device.has_correction && featureFlags.correctedCheck && (
          <div
